// @bun
// src/server/lib/postgres/repositories/api_keys.ts
...
var hashApiKey = (plaintext) => {
  return createHash("sha256").update(plaintext).digest("hex");
};
var createApiKey = async (input) => {
  ...
  return { key_id: result.key_id, plaintext, prefix };
};
var listApiKeys = async (user_id) => {
  const sql = `
  ...
  return result.rows.map((row) => {
    return new ApiKeyModel({ ...row, key_hash: "", updated: null }).toJSON();
  });
};
var revokeApiKey = async (key_id, user_id) => {
  const sql = `
...
var verifyApiKey = async (plaintext) => {
  if (!plaintext.startsWith(KEY_PLAINTEXT_PREFIX))
    return null;
  ...
  pool.query(`UPDATE api_keys SET last_used_at = CURRENT_TIMESTAMP WHERE key_id = $1`, [
    row.key_id
  ]).catch(() => {
    return;
  });
